...
namespace DrivingSchoolAppTests.Repositories
{
    ...
    public class ClassroomRepositoryTests
    {
        ...
        public async Task Check_ClassroomTracking_ReturnsClassroom()
        {
            ...
            var result = await _repository.CheckClassroomTracking(idOfClassroomToCheck);
        ...
        [TestMethod]
        public async Task Check_ClassroomTracking_ReturnsNull()
        {
            var options = new DbContextOptionsBuilder<DrivingSchoolDbContext>()
                .UseInMemoryDatabase(Guid.NewGuid().ToString())
                .Options;
        ...
        public async Task Update_Classroom_ReturnsClassroom()
        {
            ...
            var classroom1 = new Classroom { Id = 1, AddressId = 1, Number = 1, Size = 10 };
            var classroom2 = new Classroom { Id = 2, AddressId = 1, Number = 2, Size = 20 };
            var classroom1update = new ClassroomRequestDTO { AddressId = 1, Number = 10, Size = 100 };
            await _dbContext.Addresses.AddAsync(address1);
            await _dbContext.Classrooms.AddRangeAsync(classroom1, classroom2);
            await _dbContext.SaveChangesAsync();
            _repository = new ClassroomRepository(_dbContext);

            var result = await _repository.UpdateClassroom(classroom1, classroom1update);

            var updatedClassroom = await _repository.CheckClassroom(classroom1.Id);
            Assert.IsNotNull(result);
            ...
            Assert.AreEqual(1, updatedClassroom.Id);
            Assert.AreEqual(1, updatedClassroom.AddressId);
            Assert.AreEqual(10, updatedClassroom.Number);
            Assert.AreEqual(100, updatedClassroom.Size);
