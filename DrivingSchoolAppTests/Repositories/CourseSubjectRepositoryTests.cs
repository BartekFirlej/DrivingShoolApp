...
namespace DrivingSchoolAppTests.Repositories
{
    ...
    public class CourseSubjectRepositoryTests
    {
        ...
        [TestMethod]
        public async Task Check_CourseSubjectTracking_ReturnsCourseSubject()
        {
            var options = new DbContextOptionsBuilder<DrivingSchoolDbContext>()
                .UseInMemoryDatabase(Guid.NewGuid().ToString())
                .Options;
            ...
            var result = await _repository.CheckCourseSubjectTracking(idOfCourseToCheck, idOfSubjectToCheck);
        ...
        [TestMethod]
        public async Task Check_CourseSubjectTracking_ReturnsNull()
        {
            var options = new DbContextOptionsBuilder<DrivingSchoolDbContext>()
                .UseInMemoryDatabase(Guid.NewGuid().ToString())
                .Options;
        ...
        [TestMethod]
        public async Task Update_CourseSubject_ReturnsCourseSubject()
        {
            var options = new DbContextOptionsBuilder<DrivingSchoolDbContext>()
                 .UseInMemoryDatabase(Guid.NewGuid().ToString())
                 .Options;
            ...
            var courseSubject1 = new CourseSubject { CourseId = 1, SubjectId = 1, SequenceNumber = 1 };
            var courseSubject2 = new CourseSubject { CourseId = 2, SubjectId = 2, SequenceNumber = 1 };
            var courseSubject1Update = new CourseSubjectRequestDTO { CourseId = 1, SubjectId = 1, SequenceNumber = 3 };
            await _dbContext.Subjects.AddRangeAsync(subject1, subject2);
            await _dbContext.LicenceCategories.AddAsync(licenceCategory1);
            await _dbContext.CourseTypes.AddAsync(courseType1);
            await _dbContext.Courses.AddRangeAsync(course1, course2);
            ...
            var result = await _repository.UpdateCourseSubject(courseSubject1, courseSubject1Update);

            var updatedCourseSubject = await _repository.CheckCourseSubject(courseSubject1.CourseId, courseSubject1.SubjectId);
            Assert.IsNotNull(result);
            Assert.AreEqual(1, result.CourseId);
            Assert.AreEqual(1, result.SubjectId);
            Assert.AreEqual(3, result.SequenceNumber);
            Assert.IsNotNull(updatedCourseSubject);
            Assert.AreEqual(1, updatedCourseSubject.CourseId);
            Assert.AreEqual(1, updatedCourseSubject.SubjectId);
            Assert.AreEqual(3, updatedCourseSubject.SequenceNumber);
            await _dbContext.DisposeAsync();
        }
    }
}