...
namespace DrivingSchoolAppTests.Repositories
{
    ...
    public class DrivingLessonRepositoryTests
    {
        ...
        [TestMethod]
        public async Task Check_DrivingLessonTracking_ReturnsNull()
        {
            var options = new DbContextOptionsBuilder<DrivingSchoolDbContext>()
                .UseInMemoryDatabase(Guid.NewGuid().ToString())
                .Options;
        ...
        [TestMethod]
        public async Task Update_DrivingLesson_ReturnsDrivingLesson()
        {
            var options = new DbContextOptionsBuilder<DrivingSchoolDbContext>()
                .UseInMemoryDatabase(Guid.NewGuid().ToString())
                .Options;
            ...
            var drivingLesson1 = new DrivingLesson { Id = 1, AddressId = 1, CustomerId = 1, LecturerId = 1, LessonDate = new DateTime(2023, 4, 3), CourseId = 1 };
            var drivingLesson2 = new DrivingLesson { Id = 2, AddressId = 2, CustomerId = 2, LecturerId = 2, LessonDate = new DateTime(2023, 5, 6), CourseId = 2 };
            var drivingLesson1Update = new DrivingLessonRequestDTO { AddressId = 2, CustomerId = 2, LecturerId = 2, LessonDate = new DateTime(2023, 6, 6), CourseId = 2 };
            await _dbContext.CourseTypes.AddAsync(courseType1);
            await _dbContext.Courses.AddRangeAsync(course1, course2);
            await _dbContext.Lecturers.AddRangeAsync(lecturer1, lecturer2);
            await _dbContext.Customers.AddRangeAsync(customer1, customer2);
            ...
            var result = await _repository.UpdateDrivingLesson(drivingLesson1, drivingLesson1Update);

            var updatedDrivingLesson = await _repository.CheckDrivingLesson(1);
            Assert.IsNotNull(result);
            Assert.AreEqual(1, result.Id);
            Assert.AreEqual(new DateTime(2023, 6, 6), result.LessonDate);
            Assert.AreEqual(2, result.LecturerId);
            Assert.AreEqual(2, result.CustomerId);
            Assert.AreEqual(2, result.AddressId);
            Assert.AreEqual(2, result.CourseId);
            Assert.IsNotNull(updatedDrivingLesson);
            Assert.AreEqual(1, updatedDrivingLesson.Id);
            Assert.AreEqual(new DateTime(2023, 6, 6), updatedDrivingLesson.LessonDate);
            Assert.AreEqual(2, updatedDrivingLesson.LecturerId);
            Assert.AreEqual(2, updatedDrivingLesson.CustomerId);
            Assert.AreEqual(2, updatedDrivingLesson.AddressId);
            Assert.AreEqual(2, updatedDrivingLesson.CourseId);
